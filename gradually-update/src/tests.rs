...
fn gradually_update_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_has_event(Event::gradually_update(crate::Event::GraduallyUpdateAdded(
			update.key,
			update.per_block,
			update.target_value,
		)));
...
fn cancel_gradually_update_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_has_event(Event::gradually_update(crate::Event::GraduallyUpdateCancelled(
			update.key.clone(),
		)));
	});
}
...
fn add_on_finalize_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_has_event(Event::gradually_update(crate::Event::Updated(
			10,
			update.key.clone(),
