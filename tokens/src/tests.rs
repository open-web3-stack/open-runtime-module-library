...
fn fungibles_inspect_hold_trait_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				<Tokens as fungibles::InspectHold<_>>::balance_on_hold(DOT, &(), &ALICE),
				0
			);
...
fn fungibles_mutate_hold_trait_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				<Tokens as fungibles::MutateHold<_>>::release(DOT, &(), &ALICE, 40, Precision::Exact),
				Ok(40)
			);
			assert_eq!(
				<Tokens as fungibles::InspectHold<_>>::balance_on_hold(DOT, &(), &ALICE),
			...
			assert_noop!(
				<Tokens as fungibles::MutateHold<_>>::release(DOT, &(), &ALICE, 61, Precision::Exact),
				Error::<Runtime>::BalanceTooLow
			);

			// exceed hold amount when in best_effort
			assert_eq!(
				<Tokens as fungibles::MutateHold<_>>::release(DOT, &(), &ALICE, 61, Precision::BestEffort),
				Ok(60)
			);
			assert_eq!(
				<Tokens as fungibles::InspectHold<_>>::balance_on_hold(DOT, &(), &ALICE),
			...
			assert_eq!(<Tokens as fungibles::Inspect<_>>::balance(DOT, &BOB), 100);
			assert_eq!(
				<Tokens as fungibles::InspectHold<_>>::balance_on_hold(DOT, &(), &BOB),
				0
			);
			assert_eq!(
...
fn fungibles_inspect_convert_should_work() {
	...
	impl ConvertBalance<Balance, Balance> for ConvertBalanceTest {
		...
		fn convert_balance(balance: Balance, _asset_id: CurrencyId) -> Result<Balance, sp_runtime::ArithmeticError> {
			Ok(balance * 100)
		}
		...
		) -> Result<Balance, sp_runtime::ArithmeticError> {
			Ok(balance / 100)
		}
	}
