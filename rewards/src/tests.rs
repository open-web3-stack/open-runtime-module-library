...
fn transfer_share() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(RewardsModule::transfer_share(&ALICE, &DOT_POOL, 33, &BOB));
		assert_ok!(RewardsModule::transfer_share(&ALICE, &DOT_POOL, 33, &CAROL));
		...
		assert_ok!(RewardsModule::transfer_share(&BOB, &DOT_POOL, 10, &CAROL));
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(DOT_POOL, CAROL),
