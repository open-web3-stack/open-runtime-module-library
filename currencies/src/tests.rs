...
fn call_event_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &ALICE), 50);
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &BOB), 150);
			System::assert_has_event(Event::currencies(crate::Event::Transferred(X_TOKEN_ID, ALICE, BOB, 50)));
			...
			System::assert_has_event(Event::currencies(crate::Event::Transferred(X_TOKEN_ID, ALICE, BOB, 10)));
			...
			System::assert_has_event(Event::currencies(crate::Event::Deposited(X_TOKEN_ID, ALICE, 100)));

			assert_ok!(<Currencies as MultiCurrency<AccountId>>::withdraw(
				X_TOKEN_ID, &ALICE, 20
			));
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &ALICE), 120);
			System::assert_has_event(Event::currencies(crate::Event::Withdrawn(X_TOKEN_ID, ALICE, 20)));
		});
}
