...
fn multi_lockable_currency_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let _ = Currencies::set_lock(ID_1, X_TOKEN_ID, &ALICE, 50);
			assert_eq!(Tokens::locks(&ALICE, X_TOKEN_ID).len(), 1);
			let _ = Currencies::set_lock(ID_1, NATIVE_CURRENCY_ID, &ALICE, 50);
...
fn native_currency_lockable_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let _ = NativeCurrency::set_lock(ID_1, &ALICE, 10);
			assert_eq!(PalletBalances::locks(&ALICE).len(), 1);
...
fn basic_currency_adapting_pallet_balances_lockable() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let _ = AdaptedBasicCurrency::set_lock(ID_1, &ALICE, 10);
			assert_eq!(PalletBalances::locks(&ALICE).len(), 1);
			let _ = AdaptedBasicCurrency::remove_lock(ID_1, &ALICE);
			assert_eq!(PalletBalances::locks(&ALICE).len(), 0);
		});
}
