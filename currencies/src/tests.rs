...
fn call_event_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &ALICE), 50);
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &BOB), 150);
			System::assert_last_event(Event::Currencies(crate::Event::Transferred{currency_id: X_TOKEN_ID, from: ALICE, to: BOB, amount: 50}));
			...
			System::assert_last_event(Event::Currencies(crate::Event::Transferred{currency_id: X_TOKEN_ID, from: ALICE, to: BOB, amount: 10}));
			...
			System::assert_last_event(Event::Currencies(crate::Event::Deposited{currency_id: X_TOKEN_ID, who: ALICE, amount: 100}));

			assert_ok!(<Currencies as MultiCurrency<AccountId>>::withdraw(
				X_TOKEN_ID, &ALICE, 20
			));
			assert_eq!(Currencies::free_balance(X_TOKEN_ID, &ALICE), 120);
			System::assert_last_event(Event::Currencies(crate::Event::Withdrawn{currency_id: X_TOKEN_ID, who: ALICE, amount: 20}));
		});
}
